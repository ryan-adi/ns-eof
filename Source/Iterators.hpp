...
private:
  Stencils::BufferStencil& stencil_;

  const int lowOffset_;
  const int highOffset_;

public:
  ParallelBoundaryIterator(FlowFieldType& flowField, const Parameters& parameters, Stencils::BufferStencil& stencil, int lowOffset = 0, int highOffset = 0);
