...
Stencils::TurbFGHStencil::TurbFGHStencil(const Parameters& parameters):
  FieldStencil<TurbulentFlowField>(parameters) {}

void Stencils::TurbFGHStencil::apply(TurbulentFlowField& flowField, int i, int j) {
  // Load local velocities into the center layer of the local array
  loadLocalVelocity2D(flowField, localVelocity_, i, j);
  loadLocalMeshsize2D(parameters_, localMeshsize_, i, j);
  ...
  // Now the localVelocity array should contain lexicographically ordered elements around the given index
  values[0] = computeF2D(localVelocity_, localMeshsize_, localViscosity_, parameters_, parameters_.timestep.dt);
  values[1] = computeG2D(localVelocity_, localMeshsize_, localViscosity_, parameters_, parameters_.timestep.dt);
}

void Stencils::TurbFGHStencil::apply(TurbulentFlowField& flowField, int i, int j, int k) {
  // The same as in 2D, with slight modifications.
  ...
  if ((obstacle & OBSTACLE_SELF) == 0) { // If the cell is fluid
    ...
    if ((obstacle & OBSTACLE_RIGHT) == 0) { // If the right cell is fluid
      values[0] = computeF3D(localVelocity_, localMeshsize_, localViscosity_, parameters_, parameters_.timestep.dt);
    }
    if ((obstacle & OBSTACLE_TOP) == 0) {
      values[1] = computeG3D(localVelocity_, localMeshsize_, localViscosity_, parameters_, parameters_.timestep.dt);
    }
    if ((obstacle & OBSTACLE_BACK) == 0) {
      values[2] = computeH3D(localVelocity_, localMeshsize_, localViscosity_, parameters_, parameters_.timestep.dt);
    }
  }
}
