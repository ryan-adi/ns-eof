...
int main(int argc, char* argv[]) {
  ...
  //
  // FE_INVALID runs normally
  // FE_OVERFLOW runs normally
  // FE_UNDERFLOW runs normally
  // FE_DIVBYZERO "NS-EOF-Runner" received signal SIGFPE, Arithmetic exception at Simulation.cpp:124 114 or 116 depending on case
  // Runs normally after fix
  // FE_INEXACT "NS-EOF-Runner" received signal SIGFPE, Arithmetic exception.  at ./stdlib/strtod_l.c:353
  // called from   configuration.loadParameters(parameters);
  ...
  if (parameters.simulation.type == "turbulence_ml" || parameters.simulation.type == "turbulence_kw") {
    if (rank == 0) {
    ...
    throw std::runtime_error("Unknown simulation type! Currently supported: dns, turbulence_ml, turbulence_kw");
  }
