...
void Configuration::loadParameters(Parameters& parameters, const MPI_Comm& communicator) {
  ...
  if (rank == 0) {
    ...
    node    = confFile.FirstChildElement()->FirstChildElement("simulation");
    subNode = node->FirstChildElement("type");
    parameters.turb.bl_type = "non-existing";
    readStringMandatory(parameters.simulation.type, subNode);
    
    if (parameters.simulation.type == "turbulence") {
      // TODO
      readStringMandatory( parameters.turb.bl_type, node->FirstChildElement("bl_type"));
      if (parameters.turb.bl_type == "non-existing" || 
          parameters.turb.bl_type == "lam-flat" ||
          parameters.turb.bl_type == "turb-flat" ) 
      {
        // TODO
      } else {
        throw std::runtime_error("Missing boundary layer type in simulation parameters");
      }  
    }; 
  }
  ...
  broadcastString(parameters.turb.bl_type, communicator);
}
