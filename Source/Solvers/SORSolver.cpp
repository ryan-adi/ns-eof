      ...
      for (int k = 2; k < nz + 2; k++) {
        for (int j = 2; j < ny + 2; j++) {
          for (int i = 2; i < nx + 2; i++) {
            ...
            resnorm += pow(
              (flowField_.getRHS().getScalar(i, j, k) - a_W * P.getScalar(i - 1, j, k) - a_E * P.getScalar(i + 1, j, k) - a_S * P.getScalar(i, j - 1, k)
    ...
    do {
      ...
      for (int j = 2; j < ny + 2; j++) {
        for (int i = 2; i < nx + 2; i++) {
          ...
          const RealType residual = flowField_.getRHS().getScalar(i, j) - a_W * P.getScalar(i - 1, j) - a_E * P.getScalar(i + 1, j) - a_S * P.getScalar(i, j - 1)
